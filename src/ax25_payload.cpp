...
Payload::Payload(const byte *rxPayload, int payloadLength) : rptCallsCount_(0) {
  isValid_ = fromBinary(rxPayload, payloadLength);
}

Payload::Payload(const String &textPayload) : rptCallsCount_(0) {
  isValid_ = fromString(textPayload);
}

Payload::Payload(const Payload &payload)
    : isValid_(payload.isValid_), srcCall_(payload.srcCall_),
      dstCall_(payload.dstCall_), rptCallsCount_(payload.rptCallsCount_),
      info_(payload.info_) {
...
int Payload::ToBinary(byte *txPayload, int bufferLength) const {
  ...
  // destination address
  if (!dstCall_.ToBinary(txPtr, CallsignSize))
    return 0;
  txPtr += CallsignSize;
  if (txPtr >= txEnd)
    return 0;
  ...
  // source address
  if (!srcCall_.ToBinary(txPtr, CallsignSize))
    return 0;
  txPtr += CallsignSize;
  if (txPtr >= txEnd)
    return 0;
  ...
  for (int i = 0; i < rptCallsCount_; i++) {
    if (!rptCalls_[i].ToBinary(txPtr, CallsignSize))
      return 0;
    txPtr += CallsignSize;
    if (txPtr >= txEnd)
      return 0;
...
String Payload::ToString(const String &customComment) const {
  ...
  if (info_.startsWith("=") || info_.startsWith("!") || info_.startsWith("`")) {
...
bool Payload::InsertRptCallsign(const Callsign &rptCallsign, int index) {
  if (rptCallsCount_ >= RptMaxCount || index >= RptMaxCount ||
      index >= rptCallsCount_)
    return false;
...
bool Payload::fromBinary(const byte *rxPayload, int payloadLength) {
  ...
  if ((rxPtr + 2) >= rxEnd)
    return false;
  if (*(rxPtr++) != AX25Ctrl::UI)
    return false;
  if (*(rxPtr++) != AX25Pid::NoLayer3)
    return false;
...
bool Payload::fromString(const String &textPayload) {
  ...
  while (rptCallsCount_ <= RptMaxCount) {
    ...
    String pathItem =
        paths.substring(index, nextIndex == -1 ? paths.length() : nextIndex);
    if (index == 0) {
