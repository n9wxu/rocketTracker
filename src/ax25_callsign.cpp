...
Callsign::Callsign() : isValid_(false), call_(), ssid_(0) {}

Callsign::Callsign(const Callsign &callsign)
    : isValid_(callsign.isValid_), call_(callsign.call_),
...
bool Callsign::ToBinary(byte *txPayload, int bufferLength) const {
  ...
  for (unsigned int i = 0; i < CallsignSize - 1; i++) {
    if (i < call_.length()) {
...
bool Callsign::fromString(const String &inputCallsign) {
  ...
  // "ABCDEF-XX"
  if (callsign.length() > CallsignSize + 2 || callsign.length() == 0)
    return false;

  int delimIndex = callsign.indexOf('-');

  // "ABCDEF-"
  if (delimIndex != -1 && delimIndex == callsign.length() - 1)
    return false;
